...
class View(object):
    ...
    def __init__(self, line, father, sons):
        ...
        self.line=line
        self.sons=sons
        self.instance = -1  # record the number of views, complete after creating State
    ...
    def get_bounds_value(self):
        ...
        self.xmin=int(self.bounds[1:num1])
        num2=self.bounds.find("]")
        self.ymin=int(self.bounds[num1+1:num2])
        
        line=self.bounds[num2+1:len(self.bounds)]

        num1=line.find(",")
        self.xmax=int(line[1:num1])
        num2=line.find("]")
        self.ymax=int(line[num1+1:num2])
